"""
hb_integration_fit.py
=====================

Fit the Hierarchical Bayesian integration observer (hb_integration.py) and
...
Usage:
  python hb_integration_fit.py recover
  python hb_integration_fit.py human 1 3
  python hb_integration_fit.py cv 1
"""
...
def unpack(theta) -> HBIntegrationObserver:
    k_like = {0.06: np.exp(theta[0]), 0.12: np.exp(theta[1]), 0.24: np.exp(theta[2])}
    return HBIntegrationObserver(k_like=k_like, alpha=_sig(theta[3]),
                                 k_motor=np.exp(theta[4]), p_random=_sig(theta[5]),
                                 lam=_sig(theta[6]))


# --------------------------- likelihood helpers ----------------------------
def _trial_logliks(obs: HBIntegrationObserver, data):
...
def recover():
    ...
    gen = HBIntegrationObserver(**truth)
...
def _row(obs, nll, sid, n, onl, at):
    ...
    if str(sid) in at:                       # AT: switch-family, k=11
        a = at[str(sid)]["aic_at"]
        row.update(aic_at=a, bic_at=11 * np.log(n) + (a - 22))
    return row
    ...
    for sid in subject_ids:
        ...
        for name, x0 in _starts_for(onl, sid):
            ...
            if better:
                ...
                disk[str(sid)] = _row(best_obs, best_nll, sid, n, onl, at)
                json.dump(disk, open(HB_FITS, "w"), indent=2)

        row = res[str(sid)]
        rivals = [("integration", row["aic_integration"])]
        for m, disp in (("online", "online"), ("static", "static"), ("at", "AT")):
            if f"aic_{m}" in row:
