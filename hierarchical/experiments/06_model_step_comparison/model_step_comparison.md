...
Two of the six are the paper's own **non-learning** models — the **Switch**
...
the only model of the six in which *how much to trust the prior* is itself
learned — the abstract's literal requirement.
...
| Aspect | Switch | Basic Bayesian | HB - Rachel | HB - Salma | Recombined | HB - Adaptive |
|---|---|---|---|---|---|---|
| Form | Delta at 225° | Pure V(θ;225,k_prior) | α·V(θ;225,κ) + (1−α)/360 | Pure V(θ;225,κ), per κ | α·V(θ;225,κ) + (1−α)/360 | α·V(θ;225,κ) + (1−α)/360, per (κ,α) pair |
| Mixture floor (uniform) | via lapse only | via lapse only | **yes, weight (1−α)** | **no** (κ=0 grid point) | **yes, weight (1−α)** | **yes, (1−α) — and α is learned** |
| Prior width set by | fitted `k_prior`/block | fitted `k_prior`/block | learned belief b_t(κ) | learned belief H_t(κ) | learned belief b_t(κ) | **joint learned belief b_t(κ,α)** |
| Label | H2 | Eq 2 | M1 | B1 | M1 (inherited) | M1 (inherited) |

### Stage 2 — Sensory / likelihood component

| Aspect | Switch | Basic Bayesian | HB - Rachel | HB - Salma | Recombined | HB - Adaptive |
|---|---|---|---|---|---|---|
| Form | V(·;θ_true,k_like(c)) | V(·;θ_true,k_like(c)) | V(·;θ_true,k_like(c)) | V(·;θ_true,k_sensory(c)) | V(·;θ_true,k_like(c)) | V(·;θ_true,k_like(c)) |
| Measurement noise | marginalised (Girshick) | marginalised (Girshick) | marginalised (Girshick) | marginalised (Girshick) | marginalised (Girshick) | marginalised (Girshick) |
| Per-coherence reliability | 3 `k_like` | 3 `k_like` | 3 `k_like` | 3 `k_sensory` | 3 `k_like` | 3 `k_like` |
| Label | H1 | Eq 1 | M2 | B2 (grid) | M2 (inherited) | M2 (inherited) |

### Stage 3 — How prior and evidence are combined  (the defining axis)

| Aspect | Switch | Basic Bayesian | HB - Rachel | HB - Salma | Recombined | HB - Adaptive |
|---|---|---|---|---|---|---|
| Rule | **MIXTURE** — commit per trial | **INTEGRATE** — one posterior | **INTEGRATE, avg read-outs** | **INTEGRATE into one prior** | **INTEGRATE into one prior** | **INTEGRATE, avg read-outs** |
| Formula | P_e·evid + P_pr·prior + P_r·unif | MAP[V(θ_e,k_e)·V(225,k_prior)] | Σ_κ b(κ)·R_κ (avg after) | MAP(Σ_κ H(κ)·V) (one before) | MAP(Σ_κ b(κ)·V) (one before) | Σ_(κ,α) b(κ,α)·R_(κ,α) (avg after) |
| κ marginalised… | n/a | n/a | **AFTER** read-out | **BEFORE** read-out | **BEFORE** read-out | **AFTER** read-out (over κ *and* α) |
| Bimodality is… | **imposed** by commit | **none — unimodal** | **emergent** (floor) | **emergent** (posterior) | **emergent** (floor) | **emergent** (floor) |
| Label | H3+H4 | Eq 3–5 | M4 | B3+B4 | **B3-style + M's α floor** | M4 (inherited) |

### Stage 4 — Reliance-on-prior weight

| Aspect | Switch | Basic Bayesian | HB - Rachel | HB - Salma | Recombined | HB - Adaptive |
|---|---|---|---|---|---|---|
| What sets it | **derived** k_prior/(k_prior+k_e) | **emergent** (precision-weight) | **fitted** α (fixed) | emergent from belief | **fitted** α (fixed) | **learned** α (latent, tracked) |
| Free parameter? | no | no | **yes** (α) | no | **yes** (α) | **no — α is a learned latent** |
| Label | H3 | Eq 3–5 | M1 | — | M1 (inherited) | M1 (inherited) |

### Stage 5 — Learned latent (trial-by-trial dynamics)  ← where the HB models sit

| Aspect | Switch | Basic Bayesian | HB - Rachel | HB - Salma | Recombined | HB - Adaptive |
|---|---|---|---|---|---|---|
| Learns across trials? | **no** (fixed/block) | **no** (fixed/block) | **yes** — b_t(κ) | **yes** — H_t(κ) | **yes** — b_t(κ) | **yes** — b_t(κ,α) |
| WHAT is learned | — | — | prior concentration κ | prior concentration κ | prior concentration κ | **prior concentration κ AND confidence α** |
| Update rule | none | none | forget → Bayes-correct | forget → Bayes-correct | forget → Bayes-correct | forget → Bayes-correct (joint) |
| Forgetting form | — | — | **linear** (1−λ)b+λ·b₀ | **geometric** logH←ρ·logH | **linear** (1−λ)b+λ·b₀ | **linear** (1−λ)b+λ·b₀ |
| Reset | n/a | n/a | no (carried) | no (carried) | no (carried) | no (carried) |
| Label | — | — | M5 | B2 | M5 (inherited) | M5 (inherited, over κ×α) |

### Stage 6 — Read-out type

| Aspect | Switch | Basic Bayesian | HB - Rachel | HB - Salma | Recombined | HB - Adaptive |
|---|---|---|---|---|---|---|
| Read-out | MAP (Girshick) | MAP on joint posterior (once) | MAP per κ (Girshick) | MAP, **tie-aware** | MAP on effective prior (once) | MAP per (κ,α) pair (Girshick) |
| Tie handling | argmax | argmax | argmax | **ties share 1/#ties** | argmax | argmax |
| Label | H1/H2 | Eq 3–5 | M3 | B4 | M3 (inherited) | M3 (inherited) |

### Stage 7 — Motor noise + lapse

| Aspect | Switch | Basic Bayesian | HB - Rachel | HB - Salma | Recombined | HB - Adaptive |
|---|---|---|---|---|---|---|
| Motor | ⊛ V(·;0,k_motor) | ⊛ V(·;0,k_motor) | ⊛ V(·;0,k_motor) | ⊛ V(·;0,k_motor) FFT | ⊛ V(·;0,k_motor) | ⊛ V(·;0,k_motor) |
| Lapse | into Eq. 6 weights | (p+p_r·unif)/(1+p_r) | (p+p_r·unif)/(1+p_r) | (1−lapse)·p + lapse/n_θ | (p+p_r·unif)/(1+p_r) | (p+p_r·unif)/(1+p_r) |
| Label | H5 | Eq 7 | M4 | B5 | M4 (inherited) | M4 (inherited) |

### Numerical grid & fitting

| Aspect | Switch | Basic Bayesian | HB - Rachel | HB - Salma | Recombined | HB - Adaptive |
|---|---|---|---|---|---|---|
| Direction grid | 360 (1°) | 360 (1°) | 360 (1°) | **72 (5°)** | 360 (1°) | 360 (1°) |
| κ grid | n/a | n/a | ~15 points | 16 points (incl. κ=0) | ~15 points | **~15 κ × 9 α = 135 pairs** |
| Compute | linear | linear | linear | **log (logsumexp)** | linear | linear |
| Optimiser | (fitter-dependent) | (fitter-dependent) | (fitter-dependent) | Powell + LH multistart | (fitter-dependent) | (fitter-dependent) |
