...
HIERARCHICAL_ROOT = HERE.parents[2]
...
PDF_PATH = HERE / "figures" / "hb_adaptive_all_participants_learning_diagnostics.pdf"
...
def participant_page(pdf, subject_df, state_df, dists, fit_record):
    ...
    gs = fig.add_gridspec(5, 3, height_ratios=[0.8, 0.8, 1, 1, 1], hspace=0.5, wspace=0.28)
    fig.suptitle(
    ...
    ax_sd.plot(trial, state_df["believed_sd"], color="#30638e", lw=0.8, alpha=0.75, label="learned prior SD")
    ax_sd.plot(
        trial,
        pd.Series(state_df["believed_sd"]).rolling(25, min_periods=1).mean(),
        color="#003d5b",
        lw=1.6,
        label="25-trial mean",
    )
    ax_sd.step(trial, state_df["prior_std"], where="mid", color="#a23e48", lw=1.0, alpha=0.65, label="true block prior SD")
...
def block_alignment_page(pdf, block_alignment):
    ...
    ax = axes[0]
    bad = block_alignment["n_prior_std_values"] > 1
    for sid, g in block_alignment.groupby("subject_id"):
        y = int(sid)
        x = np.arange(len(g))
        labels = []
        for _, row in g.iterrows():
            label = row["prior_std_values"]
            labels.append(label)
            color = "#d1495b" if row["n_prior_std_values"] > 1 else "#4e79a7"
            ax.scatter(row["run_id"] + 100 * (row["session_id"] - 1), y, s=35, color=color)
        for _, row in g.iterrows():
            ax.text(
                row["run_id"] + 100 * (row["session_id"] - 1),
                y + 0.12,
                row["prior_std_values"],
                ha="center",
                va="bottom",
                fontsize=6,
                rotation=45,
    ...
    ax.set_xlabel("run index, offset by session")
    ax.set_title("Each point is one session/run block; label is prior_std value(s)")
    ax.grid(alpha=0.2)
